...
export class TodoComponent {
  ...
  model=new Model();
   ...
   addItem()
   {
    if(this.inputText!=""){ //Gönderilen içeriği okumak istiyorsan .value demen lazım.
      let data = { description:this.inputText , action:false};
     this.model.items.push( data);
     //LocalStorage içinde sadece string bilgiler saklanıyor.
    ...
    onActionChange(item:TodoItem){
      ...
      items.forEach(i=>{if(i.description==item.description){
  ...
  getBtnClasses(){
   return { 'disabled' :this.inputText.length==0,
